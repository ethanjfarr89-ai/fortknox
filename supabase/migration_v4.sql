-- FortKnox v4 Migration
-- Creates all missing tables (profiles, collections, styling boards, friends)
-- Safe to re-run — uses IF NOT EXISTS and catches duplicate policies

-- 1. Pieces columns (may already exist)
...
-- Add privacy_settings if table already existed without it
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS privacy_settings jsonb DEFAULT NULL;
